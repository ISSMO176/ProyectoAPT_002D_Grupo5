...
export const deshabilitarEncuesta = async (req, res) => {
  ...
  try {
    ...
    const nuevoEstado = encuesta.estado_encuesta === 'Habilitada' ? 'Deshabilitada' : 'Habilitada';
    ...
    res.status(500).json({ error: 'Error al cambiar el estado de la encuesta', details: error.message });
  }
};
